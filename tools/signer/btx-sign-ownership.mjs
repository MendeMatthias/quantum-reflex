...
//
// Generates a proof that the holder of a PQ Wallet for BTX master key controls the
...
// signature it produces are byte-identical to the PQ Wallet for BTX. It ships no
// wallet internals.
//
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const t = argv[i];
    if (t === "--random" || t === "--master-stdin" || t === "--pretty" || t === "--help") {
...
async function main() {
  ...
  if (flags.has("help") || flags.has("h")) {
    console.log(`qID Sign-In v1 reference signer (test wallet)

Usage:
  bun btx-sign-ownership.mjs --random --origin <origin> --nonce <hex> --ts <ms>

Key source (pick one):
  --random             throwaway identity generated on the spot (default)
  --master-hex <hex>   64-hex master seed
  --master-file <path> file whose first line is the 64-hex master seed
  --master-stdin       read the 64-hex master seed from stdin

Challenge (copy nonce + ts from the site's sign-in dialog):
  --origin <origin>    the site's exact web origin, e.g. https://yourapp.com
  --nonce <hex>        challenge nonce
  --ts <ms>            challenge timestamp (milliseconds)

Prints the signed proof JSON: paste it into the dialog's desktop tab, or
POST it to <origin>/qid/proof to simulate a phone scanning the QR.`);
    process.exit(0);
  }
